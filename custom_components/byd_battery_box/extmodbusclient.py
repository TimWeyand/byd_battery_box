...
class ExtModbusClient:
    ...
    async def read_holding_registers(self, unit_id, address, count, retries = 3):
        ...
        # Try several API signatures without using request classes (for broad pymodbus compat)
            ...
            try:
                try:
                    ...
                    try:
                        ...
                        try:
                            # Some variants only take (address, count)
                            data = await self._client.read_holding_registers(address, count)
                        except TypeError:
                            # Final fallback: positional with unit
        ...
        try:
            try:
                ...
                try:
                    ...
                    try:
                        # Some variants accept only (address, values)
                        result = await self._client.write_registers(address, payload)
                    except TypeError:
                        # Final fallback: positional including unit
